...
export default function DashboardPage() {
  const { user } = useAuth()
  const stats = {
    blogs: blogService.getAllPosts().length,
    media: mediaService.getAllMedia().length,
    messages: contactService.getAllMessages().length,
    ideas: ideasService.getAllIdeas().length,
  }
            ...
            >
              <Link href={action.href} className="block h-full">
                <div className="flex items-start justify-between mb-4">
                  <div className={`p-3 rounded-lg bg-secondary/50 ${action.color}`}>
                    <Icon className="w-6 h-6" />
                  </div>
                  <span className="text-2xl font-bold text-foreground">{action.count}</span>
                </div>
